...
function draw() {
  ...
  C.forEach((c) => {
    ...
    p.addEventListener("drop", (e) => {
      ...
      check(e.dataTransfer.getData("id"), c.id, p);
    });
    targets.appendChild(p);
    let t = el("text");
    t.setAttribute("x", centers[c.id][0]);
    t.setAttribute("y", centers[c.id][1]);
    t.classList.add("target-text");
...
function check(a, b, target) {
  if (a === b) {
    let q = document.querySelector(`[data-id="${a}"]`);
    ...
    document.getElementById("msg").textContent =
      `✓ Correct! ${C.find((x) => x.id === b).name} is in place.`;
    document.getElementById("msg").style.color = "#36a269";
    if (placed === C.length) setTimeout(win, 500);
  } else {
    target.classList.add("shake");
    setTimeout(() => target.classList.remove("shake"), 400);
    document.getElementById("msg").textContent =
      "Not quite—try matching the shape to its location.";
    document.getElementById("msg").style.color = "#c6246d";
  }
}
function touch(q, id) {
  let active = false;
  ...
  q.addEventListener(
    ...
    (e) => {
      ...
      if (target) check(id, target.dataset.id, target);
    },
    { passive: false },
  );
}
function update() {
  document.getElementById("progress").textContent = `${placed}/${C.length}`;
  document.getElementById("score").textContent = score;
}
function reset() {
  placed = 0;
  score = 0;
  document.getElementById("msg").textContent = "Start with any country shape.";
  document.getElementById("msg").style.color = "#746c73";
